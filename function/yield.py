...
# При вызове генератора он не возвращает все значения сразу(как простая функция), а "приостанавливается",
# запоминая состояние, и возвращает данные по мере необходимости.
...
# -----   Много Yield в Одной Функции-Генераторе   -------------------------------------------------------------------------------------------------------------------------------------------------------
...
def second():
    # yield from возвращает все значения yield из указаной функции генаратора first
    yield from first()  # -> 1 , 2
    yield 3
    yield 4
...
try:
    with open("123.txt", encoding="utf-8") as my_file:
        ...
        print(list(my_indexes))  # -> [0, 9, 18, 27, 36, 45]
...
my_number  # -> <generator object some at 0x000001B883F447D0>
